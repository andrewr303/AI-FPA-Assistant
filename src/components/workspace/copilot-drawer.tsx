...
export function CopilotDrawer({ open, onOpenChange, prefill }: { open: boolean; onOpenChange: (o: boolean) => void; prefill?: string; }) {
  ...
  return (<Sheet open={open} onOpenChange={onOpenChange}><SheetContent side="right" className="w-full sm:max-w-[520px] flex flex-col bg-card border-l border-border p-0"><SheetHeader className="px-5 pt-5 pb-3 border-b border-border"><div className="flex items-center gap-2"><div className="h-7 w-7 rounded-md bg-linear-to-br from-primary to-primary/40 flex items-center justify-center"><Sparkles className="h-3.5 w-3.5 text-primary-foreground" /></div><div><SheetTitle className="text-base">Ask FP&amp;A Expert</SheetTitle><SheetDescription className="text-[11px] uppercase tracking-widest text-muted-foreground">Agent Workspace · Live</SheetDescription></div></div></SheetHeader>
